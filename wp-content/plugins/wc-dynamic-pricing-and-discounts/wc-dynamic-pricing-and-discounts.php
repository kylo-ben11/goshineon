...
class RP_WCDPD
{
    ...
    public function on_plugins_loaded()
    {
        ...
        include_once RP_WCDPD_PLUGIN_PATH . 'includes/classes/libraries/rightpress-conditions-helper.class.php';
        include_once RP_WCDPD_PLUGIN_PATH . 'includes/classes/libraries/rightpress-wc-meta.class.php';
        ...
        foreach (glob(RP_WCDPD_PLUGIN_PATH . 'includes/classes/abstract/*.class.php') as $filename)
        {
            include $filename;
        }

        // Load controller classes
        foreach (glob(RP_WCDPD_PLUGIN_PATH . 'includes/classes/controllers/*.class.php') as $filename)
        {
            include $filename;
        }

        // Load classes
        foreach (glob(RP_WCDPD_PLUGIN_PATH . 'includes/classes/*.class.php') as $filename)
        {
            include $filename;
        }

        // Load method classes
        foreach (glob(RP_WCDPD_PLUGIN_PATH . 'includes/classes/methods/*.class.php') as $filename)
        {
            include $filename;
        }

        // Load condition classes
        foreach (glob(RP_WCDPD_PLUGIN_PATH . 'includes/classes/conditions/*.class.php') as $filename)
        {
            include $filename;
        }

        // Load condition method classes
        foreach (glob(RP_WCDPD_PLUGIN_PATH . 'includes/classes/condition-methods/*.class.php') as $filename)
        {
            include $filename;
        }

        // Load condition field classes
        foreach (glob(RP_WCDPD_PLUGIN_PATH . 'includes/classes/condition-fields/*.class.php') as $filename)
        {
            include $filename;
        }

        // Load pricing method classes
        foreach (glob(RP_WCDPD_PLUGIN_PATH . 'includes/classes/pricing-methods/*.class.php') as $filename)
        {
            include $filename;
        }

        // Load promotion tools
        foreach (array('volume_pricing_table', 'display_price_override') as $promotion_key) {
            if (RP_WCDPD_Settings::get('promo_' . $promotion_key)) {
                include RP_WCDPD_PLUGIN_PATH . 'includes/classes/promotion/rp-wcdpd-promotion-' . str_replace('_', '-', $promotion_key) . '.class.php';
            }
        }
