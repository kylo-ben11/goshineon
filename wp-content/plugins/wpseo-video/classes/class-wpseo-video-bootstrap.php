...
class WPSEO_Video_Bootstrap {
	...
	protected function can_activate() {
		if ( ! $this->is_spl_autoload_available() ) {
			$this->add_admin_notice( esc_html__( 'The PHP SPL extension seems to be unavailable. Please ask your web host to enable it.',
				'yoast-video-seo' ) );
		}

		if ( ! $this->is_wordpress_up_to_date() ) {
			$this->add_admin_notice( esc_html__( 'Please upgrade WordPress to the latest version to allow WordPress and the Video SEO module to work properly.',
				'yoast-video-seo' ) );
		}

		if ( ! $this->is_yoast_seo_active() ) {
			$this->add_admin_notice( $this->get_wpseo_missing_error(), false );
		}
		...
		if ( $this->is_yoast_seo_active() && ! $this->is_yoast_seo_up_to_date() ) {
			$this->add_admin_notice(
				sprintf(
					...
					esc_html__( 'Please upgrade the %1$s plugin to the latest version to allow the Video SEO module to work.',
						'yoast-video-seo' ),
					'Yoast SEO'
				)
			);
	 ...
	 * @return void
	 */
	protected function add_admin_notice( $message, $use_prefix = true ) {
		$prefix = '';
	...
	protected function is_yoast_seo_up_to_date() {
		return $this->is_yoast_seo_active() && version_compare( WPSEO_VERSION, '3.0', '>=' );
	}
	...
	protected function get_install_by_admin_message() {
		return sprintf(
			/* translators: %1$s expands to Yoast SEO. */
			esc_html__( 'Please ask the (network) admin to install & activate %1$s and then enable its XML sitemap functionality to allow the Video SEO module to work.',
				'yoast-video-seo' ),
			'Yoast SEO'
		);
	}
	...
	protected function get_install_plugin_message() {
		...
		return sprintf(
			...
			esc_html__( 'Please %1$sinstall & activate %2$s%3$s and then enable its XML sitemap functionality to allow the Video SEO module to work.',
				'yoast-video-seo' ),
			'<a href="' . esc_url( $url ) . '">',
			'Yoast SEO',
			'</a>'
