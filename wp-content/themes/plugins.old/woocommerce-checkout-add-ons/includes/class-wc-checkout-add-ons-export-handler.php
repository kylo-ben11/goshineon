...
class WC_Checkout_Add_Ons_Export_Handler {
	...
	protected function get_checkout_add_ons_formatted( $order ) {
		...
		foreach( $order_add_ons as $id => $add_on ) {

			$add_on_data = array();

			$add_on_data['ID']    = $id;
			$add_on_data['Name']  = $add_on['name'];
			$add_on_data['Value'] = is_array( $add_on['normalized_value'] ) ? implode( ', ', $add_on['normalized_value'] ) : $add_on['normalized_value'];
			$add_on_data['Cost']  = wc_format_decimal( $add_on['total'], 2 );
