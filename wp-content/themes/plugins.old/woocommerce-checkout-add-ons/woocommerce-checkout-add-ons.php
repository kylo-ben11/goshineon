...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.4.0', __( 'WooCommerce Checkout Add-Ons', 'woocommerce-checkout-add-ons' ), __FILE__, 'init_woocommerce_checkout_add_ons', array(
...
class WC_Checkout_Add_Ons extends SV_WC_Plugin {
	...
	/** plugin text domain, DEPRECATED in 1.7.0 */
	...
	public function get_documentation_url() {
		return 'http://docs.woothemes.com/document/woocommerce-checkout-add-ons/';
	}
	 ...
	 *
	 * @TODO Remove this as part of WC 2.7 compat {FN 2016-05-19}
	 *
	 * @since 1.8.0
	...
	public function __get( $property ) {
		...
		$main_class = 'wc_checkout_add_ons()';
		$deprecated = $main_class . '->' . $property;

		switch ( $property ) {

			case 'admin' :
				_deprecated_function( $deprecated, '1.8.0', $main_class . '->get_admin_instance()' );
				return $this->get_admin_instance();

			case 'frontend' :
				_deprecated_function( $deprecated, '1.8.0', $main_class . '->get_frontend_instance()' );
				return $this->get_frontend_instance();

			case 'export_handler' :
				_deprecated_function( $deprecated, '1.8.0', $main_class . '->get_export_handler_instance()' );
				return $this->get_export_handler();
