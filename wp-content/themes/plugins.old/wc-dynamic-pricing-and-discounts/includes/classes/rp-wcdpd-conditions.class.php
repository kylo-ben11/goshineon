...
class RP_WCDPD_Conditions
{
    ...
    public static function conditions_are_matched($conditions, $params = array())
    {
        ...
        foreach ($conditions as $condition) {
            ...
            if (!empty($params['context']) && in_array($params['context'], array('cart_discounts', 'checkout_fees'), true) && RightPress_Helper::string_begins_with_substring($condition['type'], 'product__')) {
        ...
        if ($condition = RP_WCDPD_Controller_Conditions::get_item($params['condition']['type'])) {
            ...
            if (!is_ajax()) {
                ...
                if (!isset(self::$cache[$cache_hash])) {
                    ...
                    // Check condition
                    self::$cache[$cache_hash] = call_user_func($condition['check_callback'], $params);
                }
            ...
            // Do not use caching
                ...
                // Check condition
                return call_user_func($condition['check_callback'], $params);
            }
        }
    ...
    public static function get_condition_method_options_for_display($combined_condition_key)
    {
        if ($condition = RP_WCDPD_Controller_Conditions::get_item($combined_condition_key)) {
            if ($method = RP_WCDPD_Controller_Condition_Methods::get_item($condition['method'])) {
    ...
    {
        if ($condition = RP_WCDPD_Controller_Conditions::get_item($combined_condition_key)) {
            if (isset($condition['fields']) && is_array($condition['fields']) && !empty($condition['fields'][$position])) {

                // Get total field count (all positions)
                $field_count = count($condition['fields'], COUNT_RECURSIVE) - count($condition['fields']);

                foreach ($condition['fields'][$position] as $field_key) {
                    if ($field = RP_WCDPD_Controller_Condition_Fields::get_item($field_key)) {
                        ...
                        // Print field
                        call_user_func($field['display_callback'], $context, $alias);
