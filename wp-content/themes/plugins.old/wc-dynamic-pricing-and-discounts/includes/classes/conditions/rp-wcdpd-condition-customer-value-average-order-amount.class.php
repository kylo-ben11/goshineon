...
class RP_WCDPD_Condition_Customer_Value_Average_Order_Amount extends RP_WCDPD_Condition_Customer_Value
{
    ...
    {
        return __('Average order amount within', 'rp_wcdpd');
    }
    ...
    public function get_value($params)
    {
        ...
        // Get order ids
        if ($order_ids = RightPress_Conditions_Helper::get_order_ids_by_timeframe($params['condition']['timeframe_span'])) {

            // Get amount spent
            foreach ($order_ids as $order_id) {
                if ($order = wc_get_order($order_id)) {
                    $total += (float) RightPress_WC_Legacy::order_get_total($order);
                    $number++;
                }
            }
