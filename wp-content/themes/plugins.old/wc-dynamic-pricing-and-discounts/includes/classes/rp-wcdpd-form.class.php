...
class RP_WCDPD_Form
{
    ...
    private static function attributes($params, $custom = array(), $type = 'text')
    {
        ...
        $attributes = array_merge(array('type', 'name', 'id', 'class', 'autocomplete', 'style', 'data-rp-wcdpd-validation', 'data-rp-wcdpd-hint', 'title'), $custom);
    ...
    private static function label_html($params)
    {
        ...
        if (!empty($params['id']) && isset($params['label']) && !RightPress_Helper::is_empty($params['label'])) {
            ...
            // Field is required
            // $required_html = !empty($params['required']) ? ' <abbr class="required" title="' . __('required', 'rp_wcdpd') . '">*</abbr>' : '';
