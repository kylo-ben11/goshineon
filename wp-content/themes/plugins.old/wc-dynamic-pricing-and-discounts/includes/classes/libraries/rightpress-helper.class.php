 ...
 * Main Class
 */
if (!class_exists('RightPress_Helper_12')) {

final class RightPress_Helper_12
{
    ...
    {
        // Pass through all supported currency switcher extensions
        $amount = RightPress_Helper::get_amount_in_currency_aelia($amount, $to_currency, $from_currency);
        $amount = RightPress_Helper::get_amount_in_currency_realmag777($amount);
     ...
     * @return float
     */
    public static function get_amount_in_currency_realmag777($amount)
    {
    ...
    public static function get_wc_cart_sum_of_item_quantities($params = array())
    {
        ...
        if (is_object($woocommerce) && isset($woocommerce->cart) && is_object($woocommerce->cart)) {
            ...
            if (empty($params)) {
                $sum = $woocommerce->cart->cart_contents_count;
            }
            // Filter cart items
            else {
    ...
    public static function wc_filter_cart_items($cart_items, $params)
    {
        ...
        foreach ($cart_items as $cart_item_key => $cart_item) {
            ...
            if (!empty($params['products'])) {
                if (!in_array((string) $cart_item['product_id'], $params['products'], true)) {
                    continue;
                }
            }
            ...
            if (!empty($params['product_variations'])) {
                if (empty($cart_item['variation_id']) || !in_array((string) $cart_item['variation_id'], $params['product_variations'], true)) {
                    continue;
                }
            }
            ...
            if (!empty($params['product_categories'])) {
                ...
                $cart_item_category_ids = RightPress_Helper::get_wc_product_category_ids_from_product_ids(array($cart_item['product_id']));
                $cart_item_category_ids = array_map('strval', $cart_item_category_ids);
                ...
                $matching_category_ids = array_intersect($cart_item_category_ids, $condition_category_ids);
            ...
            if (!empty($params['product_attributes'])) {
                ...
                // Get cart item attribute ids
                $cart_item_attribute_ids = RightPress_Helper::get_wc_product_attribute_ids_from_cart_item($cart_item);
                $cart_item_attribute_ids = array_map('strval', $cart_item_attribute_ids);

                // Get matching attribute ids
                $matching_attribute_ids = array_intersect($cart_item_attribute_ids, $params['product_attributes']);
            ...
            if (!empty($params['product_tags'])) {
                ...
                // Get cart item tag ids
                $cart_item_tag_ids = RightPress_Helper::get_wc_product_tag_ids_from_product_ids(array($cart_item['product_id']));
                $cart_item_tag_ids = array_map('strval', $cart_item_tag_ids);

                // Get matching tag ids
                $matching_tag_ids = array_intersect($cart_item_tag_ids, $params['product_tags']);
            ...
            // If we ended up here, cart item matches all criteria
     ...
     * @return mixed
     */
    public static function get_wc_last_user_paid_order_id($user_id = null, $billing_email = null)
    {
        // Build query
        $config = array(
            'numberposts'   => 1,
        ...
        // Use billing email
        else if ($billing_email !== null) {
            $config['meta_query'][] = array(
                'key'   => '_billing_email',
                'value' => $billing_email,
            );
        }
        // Nothing way to determine customer
        else {
            return false;
        }
