...
class RP_WCDPD_Method_Product_Pricing_Simple extends RP_WCDPD_Method_Product_Pricing
{
    ...
    public function get_adjustments($rule, $cart_items = null)
    {
        ...
        foreach ($cart_items as $cart_item_key => $cart_item) {
            ...
            if (!RP_WCDPD_Controller_Methods_Product_Pricing::is_already_processed($rule['uid'], $cart_item_key) && RP_WCDPD_Conditions::rule_conditions_are_matched($rule, array('cart_item' => $cart_item))) {
                ...
                $adjustments[$cart_item_key] = array(
                    'rule'              => $rule,
                    'reference_amount'  => $this->get_reference_amount(array('rule' => $rule), $base_price, $cart_item),
                );
            }
        }
