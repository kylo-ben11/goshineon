...
abstract class RP_WCDPD_Controller
{
    ...
    public static function get_item($key, $context = null)
    {
        if ($class = get_called_class()) {
            if ($class !== 'RP_WCDPD_Controller') {
                ...
                return $controller->get_own_item($key, $context);
            }
        }
    ...
    public static function get_items($context = null)
    {
        if ($class = get_called_class()) {
            if ($class !== 'RP_WCDPD_Controller') {
                $controller = $class::get_instance();
                return $controller->get_own_items($context);
            }
        }
    ...
    public static function get_items_for_display($context = null)
    {
        if ($class = get_called_class()) {
            if ($class !== 'RP_WCDPD_Controller') {
                $controller = $class::get_instance();
                return $controller->get_own_items_for_display($context);
            }
        }
     ...
     * @return mixed
     */
    public function get_own_item($key, $context = null)
    {
        $item = false;
        ...
        // Check by context
        if ($item !== false && $context !== null && (!isset($item['contexts']) || !in_array($context, $item['contexts'], true))) {
            return false;
        }
     ...
     * @return array
     */
    public function get_own_items($context = null)
    {
     ...
     * @return array
     */
    public function get_own_items_for_display($context = null)
    {
        $result = array();
        ...
        foreach ($items as $item_group_key => $item_group) {
            foreach ($item_group['children'] as $item_key => $item) {
                ...
                // Add item
                if ($this->items_are_grouped) {
                    $result[$item_group_key]['options'][$item_key] = $item['label'];
                }
                else {
                    $result[$item_key] = $item['label'];
                }
            }
        }
    ...
    public function filter_items_by_context($items, $context)
    {
        ...
        foreach ($items as $item_group_key => $item_group) {
            ...
            foreach ($item_group['children'] as $item_key => $item) {
                ...
                // Check context
                if (!isset($item['contexts']) || !in_array($context, $item['contexts'], true)) {
    ...
    {
        return (bool) $this->get_own_item($key, $context);
    }
    ...
    {
        foreach ($this->get_items() as $potential_group_key => $potential_group) {
